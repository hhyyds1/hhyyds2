...
<html><head><meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
  ...
  <body>
    ...
    var点=心上点(t)；
    point.x=
    settings.particle.size/2+
    (point.x*settings.particle.size)/350；
    point.y=
    settings.particle.size/2-
    (point.y*settings.particle.size)/350；
    返回点；
          }
    //创建路径
    context.beginPath()；
...
语境。clearRect(0，0，画布.宽度，画布.高度)；
    ...
    //创建新粒子
    var数量=particleRate*deltaTime；
    for(var i=0；i<数量；我++){
    var pos=心点(Math.PI-2*Math.PI*Math.random())；
    var dir=位置克隆()。长度(设置。粒子。速度)；
    particles.add(
    );
>
</
...
<div身份标识="___FEISHU_CLIPPER___"></div></身体></超文本标记语言>___FEISHU_CLIPPER___"></div></身体></超文本标记语言>身体</超文本标记语言>
